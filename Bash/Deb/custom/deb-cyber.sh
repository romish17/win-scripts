...
sudo apt install xfce4 -yy
sudo apt install xserver-xorg-input-multitouch xserver-xorg-input-synaptics -yy

git clone https://github.com/R0M-0X/dracula-theme.git

sudo cp -r assets /usr/share/
sudo cp -r assets/Nordic-standard-buttons-v40 /usr/share/themes/
sudo cp -r assets/wallpapers /usr/share/
sudo cp -rf assets/avatar /usr/share/lightdm/
